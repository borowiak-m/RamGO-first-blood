...
func ParseTime(input string) (Time, error) {
	...
	if len(components) != 3 {
		...
		if err != nil {
			return Time{}, &TimeParseError{fmt.Sprintf("Error parsing hours: %v", err), input}
		}
		...
		if minute > 59 || minute < 0 {
			return Time{}, &TimeParseError{"minute out of range: 0 - 59", fmt.Sprintf("%v", minute)}
		}

		if second > 59 || second < 0 {
			return Time{}, &TimeParseError{"second out of range: 0 - 59", fmt.Sprintf("%v", second)}
		}
